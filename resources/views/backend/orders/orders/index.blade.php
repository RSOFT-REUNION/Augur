        ...
        {{--    <div class="col">
                            ...
                            <div class="row justify-content-evenly">
                                <div class="col card border-left-primary m-2 p-0">
                                    ...
                                    <div class="card-body">
                                        <h2 class="card-text text-center text-primary">
                                            {{ $orders->where('status', 'paiement accepté')->count() }}</h2>
                                    </div>
                                </div>
                                <div class="col card card border-left-info m-2 p-0">
                                    ...
                                    <div class="card-body">
                                        <h2 class="card-text text-center text-info">
                                            {{ $orders->where('status', 'en cours de préparation')->count() }}</h2>
                                    </div>
                                </div>
                                <div class="col card border-left-warning m-2 p-0">
                                    ...
                                    <div class="card-body">
                                        <h2 class="card-text text-center text-warning">
                                            {{ $orders->where('status', 'prêt pour livraison')->count() }}</h2>
                                    </div>
                                </div>
                            ...
                            @foreach ($orders as $order)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <div class="card
                                        @if ($order->status->status == 'en attente de paiement') text-bg-secondary
                                        @elseif ($order->status->status == 'paiement accepté')
                                        text-bg-primary
                                        @elseif ($order->status->status == 'en cours de préparation')
                                        text-bg-info
                                        @elseif ($order->status->status == 'prêt pour livraison')
                                        text-bg-warning
                                        @elseif ($order->status->status == 'en cours de livraison')
                                        text-bg-dark
                                        @elseif ($order->status->status == 'livré')
                                        text-bg-success
                                        @elseif ($order->status->status == 'annulé')
                                        text-bg-danger
                                        @elseif ($order->status->status == 'remboursé')
                                        text-bg-light @endif
                                        "
                                    ...
                                    <td class="text-center">
                                        ...
                                        <a href="{{ route('backend.orders.orders.details', $order->id) }}"
                                            class="btn btn-warning btn-sm"><i class="fa-solid fa-location-dot"></i></a>
